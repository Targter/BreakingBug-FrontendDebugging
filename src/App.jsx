...
// Removed the import for getProducts as it is no longer used
...
const App = () => {
  ...
  const baseUrl = process.env.REACT_APP_BASE_URL; // Setting the base URL from environment variables
  const { isLoggedIn, currentToken, currentRole, productData } = useSelector(
    (state) => state.user // Accessing user state from the Redux store
  );
  // Logging user authentication status and token for debugging purposes
  useEffect(() => {
    dispatch(isTokenValid()); // Dispatching isTokenValid action to validate user token on component mount
  }, [dispatch]);
